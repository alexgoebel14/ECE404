...
char * clientComm(int clntSockfd,int * senderBuffSize_addr, int * optlen_addr){
    ...
    /*The issue was using strcpy(str, recvBuff). This would try and put too many bytes into str which was only allocated with MAX_DATA_SIZE bytes, which in this case was 5. The way I fixed it was by checking the size of the senderBuff and if that was greater than MAX_DATA_SIZE I just exited the program*/
