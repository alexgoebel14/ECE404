...
#key_file: String of a file name containing the encryption key (in ASCII) for AES
...
def x931(v0, dt, totalNum, key_file):
    ...
    #First random number
    dtEncrypt = encrypt(dt, key_bv)
    
    newX = dtEncrypt ^ v0
    R = encrypt(newX, key_bv)
    randNum = R.int_val()
...
def encrypt(x, key_bv):
    
    #Variable for our encrypted BitVector
